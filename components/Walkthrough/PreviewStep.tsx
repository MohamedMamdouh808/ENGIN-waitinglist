...
export function PreviewStep() {
  return (
    <div className="mx-auto max-w-lg">
      <p className="mb-4 text-center text-sm text-muted">Generated from your Blueprint.</p>
      <div className="overflow-hidden rounded-sm border border-line">
        <div className="flex items-center gap-1.5 border-b border-line bg-raised px-3 py-2">
          <span className="h-2 w-2 rounded-full bg-line" />
          <span className="h-2 w-2 rounded-full bg-line" />
          <span className="h-2 w-2 rounded-full bg-line" />
          <span className="ml-2 font-mono text-xs text-muted">restaurant-booking.local</span>
        </div>
          ...
          <div className="mt-3 space-y-2">
            {RESTAURANTS.map((r) => (
              ...
              >
                ...
                <span className="rounded-sm bg-accent px-3 py-1.5 text-xs font-medium text-bg">
                  Reserve
                </span>
              </div>
            ))}
